...
/* =============================================================
   1.  دوال تسجيل الدخول وإنشاء الحساب (أول واحد يصير مدير)
   ============================================================= */
...
window.resetPassword = async () => {
    ...
    if(email){ try { await sendPasswordResetEmail(auth, email); alert("تم إرسال رابط الاستعادة لبريدك!"); } catch { alert("خطأ في إرسال البريد."); } }
}
...
window.handleSignUp = async () => {
    ...
    try {
        ...
        const role = usersSnap.empty ? "admin" : "user"; // أول واحد يصير مدير
    ...
    } catch(error){ alert(error.code === 'auth/email-already-in-use' ? "البريد مستخدم!" : "خطأ في التسجيل: " + error.message); }
}
...
window.cropAndUpload = async () => {
    ...
    canvas.toBlob((blob) => { newAvatarFile = blob; document.getElementById('crop-modal').style.display = 'none'; alert("تم قص الصورة! اضغط حفظ لإتمام الرفع."); });
}

/* =============================================================
   3.  صفحة الإعدادات الشخصية
   ============================================================= */
...
window.saveUserSettings = async () => {
    ...
    try {
        ...
        alert("تم حفظ التغييرات بنجاح!"); location.reload();
    } catch(error){ if(error.code === 'auth/requires-recent-login') alert("سجل الخروج وأعد الدخول لتعديل البريد أو كلمة المرور."); else alert("حدث خطأ أثناء الحفظ."); }
}

/* =============================================================
   4.  شام كاش (إضافة رصيد)
   ============================================================= */
...
window.completeShamCashTopup = async () => {
    ...
    alert("تم إرسال طلب الشحن للموافقة!"); document.getElementById('page-topup').style.display = 'none'; document.getElementById('page-home').style.display = 'block';
}

/* =============================================================
   5.  طلبات الشحن (للمدير والمعاون فقط)
   ============================================================= */
function loadTopups(listId) {
    ...
    onSnapshot(q, (snap) => {
        ...
        snap.forEach(d => {
            const data = d.data();
            list.innerHTML += `<div style="background:#2a2a2a; padding:10px; margin:5px; border-radius:8px;">
                مستخدم: ${data.userId} - مبلغ: $${data.amount}
                <button class="btn-approve" onclick="handleTopup('${d.id}', 'approved', ${data.amount}, '${data.userId}')">قبول</button>
                <button class="btn-reject" onclick="handleTopup('${d.id}', 'rejected', 0, '')">رفض</button>
            </div>`;
        });
    });
}
window.handleTopup = async (docId, status, amount, userId) => {
    ...
    if(status === 'approved') { await updateDoc(doc(db, "users", userId), { balance: increment(amount) }); alert("تم قبول الشحن وإضافة الرصيد."); } else alert("تم رفض الطلب.");
}

/* =============================================================
   6.  إدارة المنتجات (المشرف)
   ============================================================= */
...
function loadMyProducts(uid) {
    ...
    onSnapshot(q, (snap) => {
        ...
        snap.forEach(d => {
            const p = d.data();
            list.innerHTML += `<div style="background:#2a2a2a; padding:10px; margin:5px; border-radius:8px;">${p.name} - $${p.price}
                <input type="number" id="update-${d.id}" value="${p.price}" style="width:80px;">
                <button class="btn-primary" onclick="updateMyProduct('${d.id}')">تعديل السعر</button>
            </div>`;
        });
    });
}
...
function loadShopProducts() {
    onSnapshot(collection(db, "products"), (snap) => {
        ...
        snap.forEach(d => {
            const p = d.data();
            list.innerHTML += `<div style="background:#2a2a2a; padding:10px; margin:5px; border-radius:8px; display:inline-block; width:45%;">
                <b>${p.name}</b><br>$${p.price}
                <button onclick="buyProduct('${d.id}', ${p.price})" style="width:100%; margin-top:5px;" class="btn-primary">شراء</button>
            </div>`;
        });
    });
}
window.buyProduct = async (id, price) => {
    ...
    currentUser.balance -= price; document.getElementById('user-balance').textContent = `$${currentUser.balance}`; alert("تم الشراء بنجاح!");
}
...
function loadUsersList() {
    onSnapshot(collection(db, "users"), (snap) => {
        ...
        snap.forEach(d => {
            const u = d.data();
            list.innerHTML += `<div style="background:#2a2a2a; padding:10px; margin:5px; border-radius:8px;">${u.email} (${u.role})
                <button class="btn-danger" onclick="toggleBan('${d.id}', ${!u.isBanned})">${u.isBanned ? 'فك حظر' : 'حظر'}</button>
                <button class="btn-primary" onclick="setRole('${d.id}', 'supervisor')">جعله مشرف</button>
                <button class="btn-primary" onclick="sendPasswordResetAdmin('${u.email}')">إعادة كلمة المرور</button>
            </div>`;
        });
    });
}
...
window.sendPasswordResetAdmin = async (email) => { if(confirm(`إعادة تعيين كلمة المرور لـ ${email}؟`)) { await sendPasswordResetEmail(auth, email); alert("تم إرسال رابط إعادة التعيين."); } }

/* =============================================================
   9.  التوزيع النهائي للصلاحيات (لوحة التحكم الرئيسية)
   ============================================================= */
...
function renderApp(user) {
    ...
    // إخفاء لوحات الإدارة وإظهار لوحة الدور الصحيح فقط
    document.getElementById('page-admin').style.display = 'none';
    document.getElementById('page-coadmin').style.display = 'none';
    document.getElementById('page-supervisor').style.display = 'none';
    
    if (user.role === 'admin') {
        document.getElementById('page-admin').style.display = 'block';
        loadTopups('admin-topups'); loadUsersList();
    } else if (user.role === 'co_admin') {
        document.getElementById('page-coadmin').style.display = 'block';
        loadTopups('coadmin-topups');
    } else if (user.role === 'supervisor') {
        document.getElementById('page-supervisor').style.display = 'block';
        loadMyProducts(user.uid);
    }
    loadShopProducts(); // يعرض المنتجات للكل
}
